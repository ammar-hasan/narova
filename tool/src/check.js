 ...
 * Table rows are read by named column: the claim text comes from the column
 * whose header cell matches "claim"/"as spoken" (case-insensitive substring);
 * when no header names the claim, the second column is used. Cells keep their
 * split positions — an empty cell must never shift a row's columns. */
function readClaimsLedger(config) {
  ...
  for (const name of ['claims.md', 'CLAIMS.md']) {
    ...
    if (fs.existsSync(p)) {
      ...
      let claimCol = null;   // resolved claim-column index for the open table
      const separator = l => /^\|[-|\s]+\|$/.test(l);
      // Split a table row without dropping empty cells; only the artifacts of
      // the leading/trailing pipe are discarded.
      const splitRow = (l) => {
        const cells = l.split('|').map(c => c.trim());
        if (cells.length && cells[0] === '') cells.shift();
        if (cells.length > 1 && l.endsWith('|') && cells[cells.length - 1] === '') cells.pop();
        return cells;
      };
      const pushCell = (cells, col) => {
        const cell = cells[col];
        if (typeof cell === 'string' && cell.length > 4) claimLines.push(cell);
      };
      // A row immediately followed by a separator row is a header — resolve
      // the claim column from it (first header cell naming the claim, else the
      // second column). Every header re-resolves, so multiple tables under the
      // same `## Claims` heading each contribute by their own named column.
      const resolveHeader = (row) => {
        const idx = splitRow(row).findIndex(c => /claim|as spoken/i.test(c));
        claimCol = idx >= 0 ? idx : 1;
      };
      for (let i = 0; i < lines.length; i++) {
        ...
        if (l.startsWith('## claim:') || l.startsWith('## Claim:')) {
          ...
          inClaimsTable = true;
          claimCol = null;
        } else if (inClaimsTable && separator(l)) {
          // Separator row (e.g. |---|---|) — nothing to collect. The header it
          // follows already resolved claimCol, and the next header re-resolves.
        } else if (inClaimsTable && l.startsWith('|') && next && separator(next)) {
          resolveHeader(l);
        } else if (inClaimsTable && l.startsWith('|')) {
          if (claimCol == null) claimCol = 1; // headerless table: second column
          pushCell(splitRow(l), claimCol);
        } else if (l.startsWith('- ')) {
          // Bullet entries are collected regardless of their position in the
          // file — a `## Claims` table that ends does not stop later bullets
          // from counting (NAR-007-029).
 ...
 * the generated composition structure and silently break layout. The documented
 * restylable chrome surface (progress, topbar, wordmark, counter —
 * scene-script.md §Chrome) is deliberately NOT reserved here: authors are
 * directed to restyle it in theme.css, so warning on it would punish following
 * the guidance (NAR-007-050). */
const HF_RESERVED_CLASSES = new Set([
  'scene', 'clip', 'chrome', 'overlay', 'canvas', 'scenebody', 'capzone',
  'cap-stage', 'cap-group', 'spk', 'cap-w', 'caption2', 'marklayer',
  'walkthrough-media', 'walkthrough-shell', 'walkthrough-titlebar',
  'walkthrough-dots', 'has-walkthrough', 'walkthrough-window',
  'walkthrough-full',
]);
...
function check(config, opts = {}) {
  ...
  // Zero-claims ledger advisory (NAR-007-049): a ledger file that parsed to
  // zero claim lines is loud at every level — it never changes the result,
  // and it is a separate diagnostic from the coverage statement above.
  if (ledger && ledger.claims.length === 0) {
    warnings.push(
      `${path.relative(config.projectDir || '.', ledger.path)}: no claim lines were parsed from the ledger — ` +
      'check collects "## claim:" headings, bullets anywhere in the file, and table rows under a "## Claims" heading ' +
