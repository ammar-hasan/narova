...
# <prefix>/lib/node_modules/@narova/narova. Retain the old unscoped layout so
# pre-npm GitHub installations can still remove themselves during migration.
...
SCOPED_PACKAGE_DIR="$PREFIX/lib/node_modules/@narova/narova"
LEGACY_PACKAGE_DIR="$PREFIX/lib/node_modules/narova"
if [ -d "$SCOPED_PACKAGE_DIR" ]; then
  PACKAGE_DIR="$SCOPED_PACKAGE_DIR"
  PACKAGE_NAME="@narova/narova"
...
npm uninstall --global --prefix "$PREFIX" --no-audit --no-fund --dry-run=false "$PACKAGE_NAME"
