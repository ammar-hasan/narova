...
| `narova compile` | compile `reel.config.*` → `out/timeline.json` (versioned intermediate representation). The timeline is a self-contained snapshot of every datum the pipeline needs — also written automatically by `synth`, `compose`, and `build`. | instant |
| `narova synth` | Python TTS → `out/audio/*.wav`, `out/audio/full.wav`, `out/timings.json`. Creates the venv on first run. Writes and enriches `timeline.json` with measured word timings. | piper: fast; xtts/qwen/chatterbox: slow + one-time 1–2GB model |
