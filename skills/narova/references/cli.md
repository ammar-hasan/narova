...
| `narova ingest <local.pdf> --pages <1,3-5>` | render only the selected one-based physical pages at 144 dpi, preserve available literal embedded text, update `sources.md`/`claims.md`, and register the page evidence. Selection order is preserved. No OCR, links, document actions, interpretation, or network access. | bounded local decode + render |
...
| `narova generate "<prompt>" [--provider <name>] [--continuity <shot-id>]` | ask one explicitly registered `narova-video-provider/v1` worker to create a staged clip, then transactionally publish the validated media, versioned generation recipe, hashes, and asset-registry entry. Optional continuity selects one creator-authored shot from the project config; an image anchor is sent only when the provider declares `referenceImages:true`, otherwise generation fails before provider work. Defaults to provider name `sora`; a missing registration fails before network use. | provider-defined billed network operation |
