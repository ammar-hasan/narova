...
| `narova witness [--video <file>] [--output <file>] [--json]` | first-class, built-in visual perceptual evidence over one encoded artifact. Atomically writes `out/witness.json` by default, exactly binds artifact and bundle identities, and reports luma/frame-difference/edge series plus honest unavailable privileged channels. The pixels-only path works for artifacts from both bundled renderers. No score, taste judgement, gate, mutation, model, VLM, or network call; the output is evidence, not a Judge verdict. | bounded local decode |
| `narova judge [--video <file>] [--plan] [--json]` | read-only Video CI mirror over one self-contained encoded artifact (default `out/video.mp4`). Invokes built-in Witness for visual facts, then joins them with explicit assertions, timing, audio, captions, streams, hierarchy proxies, and bound production state; it does not independently recompute visual perception. Build receipts bind shared context to the matching video digest, while unbound context stays unavailable. Indirect playlists and attached artwork are rejected. Accounts for five perception families and marks unavailable semantic perception `UNCERTAIN`. `--plan` adds plural, unranked options for assertion-linked divergence/uncertainty, always including keeping unchanged; it selects and executes nothing. No universal score, taste gate, mutation, or network access. | bounded local decode |
...
- `--video <file>` — `witness`, `judge`, or focused `branch save`: inspect this local,
...
- `--output <file>` — `witness`: project-relative atomic bundle destination in
  an existing directory; default `out/witness.json`.
