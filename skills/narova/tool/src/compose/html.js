...
const { THREE_LOCAL, GLTF_LOADER_LOCAL, threeSceneBody, hasThreeScenes, hasThreeModels } = require('./three');
...
function composeDoc(config, size, data, css) {
  ...
  const useGltf = hasThreeModels(config);
  const threeScripts = useThree
    ? `\n  <script src="${THREE_LOCAL}"></script>${useGltf ? `\n  <script src="${GLTF_LOADER_LOCAL}"></script>` : ''}`
    : '';
